...
/**For the following variables, we want to define -9 as the only missing value. The RECODE statement below will recode all existing missing values (-1, -2, -3, -4, -5, -6, -7, -8) to (-9 to facilitate this)
...
/**Now that the missing data has been recoded to -9, we can define -9 as a missing value for this dataset**/.  
...
/*In the raw dataset, each Client_ID is surrounded by single quotes These need to be removed to create a valid numeric ID.
...
/***The ObservationDate variable needs to be computed from either the InterviewDate or the DischargeDate variable
/***The earliest ObservationDate will be the Discharge date, subsequent ObservationDates will be interview dates
                            ...
                            /***Other dates in the file are improperly set as Strings or other invalid types. The blocks below will convert them to proper DATE variables**/.
                            AUTORECODE VARIABLES=GAFDate
                            /into GAFDateTemp
                            /print.
                            
                            STRING GAFDate_String (A10).
                            DO IF  (GAFDateTemp > 2).
                            ...
                            /******SIMULATE COMPUTING OF VARIABLES THAT I DONT HAVE BECAUSE THEY ARE PHI. COMMENT OUT AND RETURN ORIGINAL FOR PRODUCTION****/.
                            RECODE DateBloodDrawn(ELSE=COPY) INTO DateBloodDrawn_New.
                            EXECUTE.
                            ...
                            *STRING DateBloodDrawn_String (A10).
                            ...
                            *DELETE VARIABLES DateBloodDrawnTemp DateBloodDrawn_String .
                            ...
                            /******SAME AS ABOVE THIS IS PHI SO CAN'T WORK WITH IT******/.
                            RECODE DOB(ELSE=COPY) INTO DOB_New.
                            EXECUTE.
...
/**Some variables are binary (yes/no, true/false, etc). Variables that are binary should always be coded S.T. 0=False/No, 1=True/Yes.
/**Perform recoding to achieve this result**/.
