...
/************CHANGES MADE BELOW:
/************INSTEAD OF RECODING Assessment into FollowupXm, RECODE ConductedInterview
/************THIS WILL ALLEIVIATE THE PROBLEM OF 39 CLIENTS INCORRECTLY GETTING MARKED AS HAVING THEIR 3M FOLLOWUP WHEN THEY IN FACT MISSED IT************/.
/*Create variable to indicate 3-month follow-up*/.
DATASET ACTIVATE FullData.
SAVE OUTFILE= 'procStepFolder\Followup.sav'
  /KEEP Client_ID InterviewType_07 Assessment ConductedInterview.
EXECUTE.

GET
FILE='procStepFolder\Followup.sav'.
...
/*************THE 39 PROBLEMATIC ROWS START TO BECOME A PROBLEM HERE************/.
/*************PROBLEM IS THAT BECAUSE THE ObservationDate VALUE IS MISSING AT INTERVIEW POINT 301, 3M REASSESSMENT IS MADE TO SEEM EARLIER IN TIME THAN BL************/.
/*************IN OTHER WORDS FOR TIME = 1 AT 3M, and TIME=2 at BL. THIS IS THE OPPOSITE OF HOW IT SHOULD BE************/.
/*************SOLUTION IS TO COMPUTE A VALID OBSERVATION DATE IN STEP 1 FOR THESE CLIENTS. CAN USE MONTH AND FFY AS A FIX************/.
DATASET ACTIVATE FullData.
* Identify Duplicate Cases.
SORT CASES BY Client_ID(A) ObservationDate(A).
MATCH FILES
