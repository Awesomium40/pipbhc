...
DO IF Clinic LE 2.
    ...
    DO IF Time = 2.
        SELECT IF (Time_Cont GE 150 AND Time_Cont LE 210).
    END IF.
...
DO IF Clinic EQ 1.
    ...
    DO IF (T_Time EQ 1 AND Time_Cont GT 30).
        RECODE T_Time (1=2).
    END IF.

    DO IF (T_Time EQ 2 AND Time_Cont LT 30).
        RECODE T_Time (2=1).
    END IF.

    DO IF (T_Time = 2).
        SELECT IF (Time_Cont LE 210).
    END IF.

    DO IF (T_Time EQ 2 AND Time_Cont GT 150).
        RECODE T_Time (2=3).
    END IF.

    DO IF (T_Time EQ 2).
        SELECT IF (Time_Cont LE 120).
    END IF.

    DO IF (T_Time EQ 3).
        SELECT IF (Time_Cont GE 150).
    END IF.
END IF.
EXECUTE.
...
/*This section is making incorrect calculations for Clinic 3 because of the extra assessments at 3, 9 M*/.
DATASET ACTIVATE PIPBHC_Long.
